...
document.getElementById('search').addEventListener('input', e=>{
  ...
  let vis=0;
  rows.forEach(r=>{
    const ok = (r.dataset.text||'').includes(q);
    r.style.display= ok?'flex':'none';
    if(ok) vis++;
  });
  let empty=document.querySelector('.empty');
  if(vis===0){ if(!empty){empty=document.createElement('div'); empty.className='empty'; empty.textContent='Nessun risultato.'; document.getElementById('list').appendChild(empty);} }
...
const fOrig=document.getElementById('f-orig');
const fId=document.getElementById('f-id');
const fTit=document.getElementById('f-titolo');
const fAbbr=document.getElementById('f-nomeabbr');
const fDur=document.getElementById('f-durata');
const fAula=document.getElementById('f-aula');
const fFad=document.getElementById('f-fad');
const fVal=document.getElementById('f-validita');
const fMax=document.getElementById('f-maxp');
const fCat=document.getElementById('f-categoria');
const fTip=document.getElementById('f-tipologia');
const fNorm=document.getElementById('f-normativa');
const fProgText=document.getElementById('f-programma-text');

document.getElementById('btn-add').addEventListener('click', ()=>{
  document.getElementById('mod-title').textContent='Aggiungi corso';
  form.reset();
  fOrig.value='';
...
document.querySelectorAll('.edit').forEach(b=>{
  b.addEventListener('click', async e=>{
    ...
    const fd=new FormData(); fd.append('ajax','get_course'); fd.append('id',id);
    const j=await (await fetch(location.href,{method:'POST',body:fd})).json();
    if(!j.ok){ alert('Errore caricamento corso'); return; }
    const c=j.data||{};
    document.getElementById('mod-title').textContent='Modifica corso';
    fOrig.value=c.id||'';
    fId.value=c.id||'';
    fTit.value=c.titolo||'';
    fAbbr.value=c.nomeabbreviato||'';
    fDur.value=c.durata||'';
    fAula.checked = (c.aula==1);
    fFad.checked  = (c.fad==1);
    fVal.value = (c.validita ?? '');
    fMax.value = c.maxpartecipanti||0;
    fCat.value = c.categoria||'';
    fTip.value = c.tipologia||'';
    fNorm.value= c.normativa||'';
    fProgText.value = c.programma || '';
...
document.getElementById('btn-del').addEventListener('click', async ()=>{
  ...
  fd.append('ajax','delete_course');
  fd.append('csrf','<?=htmlspecialchars($_SESSION['csrf'],ENT_QUOTES)?>');
  fd.append('id', fOrig.value||fId.value);
  const j=await (await fetch(location.href,{method:'POST',body:fd})).json();
  if(!j.ok){ alert(j.msg||'Errore tecnico'); return; }
  location.reload();
});
...
form.addEventListener('submit', async e=>{
  e.preventDefault();
  if(!fId.value.trim() || !fTit.value.trim() || !fAbbr.value.trim()){
    alert('Compila ID, Titolo e Nome abbreviato.'); return;
  }
  const fd=new FormData(form);
  fd.append('ajax','save_course');
  const j=await (await fetch(location.href,{method:'POST',body:fd})).json();
  if(!j.ok){ alert(j.msg||'Errore tecnico'); return; }
  location.reload();
});
...
document.getElementById('btn-prog-pdf').addEventListener('click', ()=>{
  if(!fAbbr.value.trim()){
    alert('Inserisci prima il Nome abbreviato.'); return;
  }
  const df=document.getElementById('download-form');
  df.elements['titolo'].value = fTit.value||'';
  df.elements['nomeabbreviato'].value = fAbbr.value||'programma';
  df.elements['categoria'].value = fCat.value||'';
  df.elements['tipologia'].value = fTip.value||'';
  df.elements['durata'].value = fDur.value||'';
  df.elements['normativa'].value = fNorm.value||'';
  df.elements['programma_text'].value = fProgText.value||'';
  df.submit(); // apre nuova scheda con download
});
</script>
</body>
</html>
